...
| q_proj | Interaction-aware joint G50 depth + q_proj quant | 1.400x | 11.087 | 14.133 | 12.223 | 0.643 |
...
- The new interaction-aware `q_proj` joint G50 run is the best perplexity point found so far at 1.400x compression: WikiText2 PPL 11.087, C4 PPL 14.133, FineWeb-Edu PPL 12.223, and LM-eval macro 0.643.
- Relative to the previous standard `q_proj` joint G50 run, interaction-aware mutation improves perplexity by 0.157 WikiText2 PPL, 0.260 C4 PPL, and 0.237 FineWeb-Edu PPL. Its LM-eval macro is essentially tied: +0.001.
...
The evidence supports a nuanced claim rather than a simple win. Joint search consistently helps relative to independently combining depth and quantization at the same scope, especially in perplexity. The interaction-aware mutation operator strengthens the `q_proj` perplexity result and gives a concrete implementation contribution, but it does not create a clear downstream-task win. Expanding the quantization scope from `q_proj` to full attention projections increases compression while reducing model quality. The strongest thesis framing is therefore a compression-quality tradeoff: EvoPress-style joint search can recover part of the quality lost by broader combined compression, but the chosen scope and target compression strongly determine whether the result is competitive with depth-only pruning.
...
- `results/mistral_interactionaware_lmeval_aggregate.csv`
