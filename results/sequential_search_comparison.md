...
This report compares the four sequential-initialization variants against the existing Mistral q-projection baselines. Lower WikiText2 perplexity (PPL) is better.

## Scope and Validation

All sequential runs use `mistralai/Mistral-7B-v0.3`, 25% separate attention/MLP depth sparsity, a 3-bit active q-projection budget, `group_rule=size`, 20 stage-two generations, 16 offspring, 32 requested initial candidates, and seeds 0–2.

The generator verified all 12 sequential summaries, exact depth counts, active quantization budgets, stage-one provenance hashes, and frozen-component invariants. Quantization-first warm starts used strict initialization and changed zero imported genes before initial selection.
...
**Depth → Joint, warm-started is the best sequential variant at 11.526 ± 0.223 PPL.**
...
This is a 3.52% mean PPL improvement over independent composition and a 8.57% improvement over standard joint G20. Interaction-aware G50 reaches the lowest reference mean (11.086), but uses a different mutation operator and a 50-generation single-search budget.
...
| Depth → Joint, warm-started | new sequential variant | 3 | 11.526 ± 0.223 | 9.58 | 8.98 | 18.56 |
| Quantization → Depth, frozen | new sequential variant | 3 | 14.003 ± 2.748 | 13.15 | 8.64 | 21.79 |
| Quantization → Joint, warm-started | new sequential variant | 3 | 12.573 ± 0.439 | 13.15 | 9.01 | 22.16 |
...
| Seed | Depth→Quant frozen | Depth→Joint warm | Quant→Depth frozen | Quant→Joint warm | Independent | Joint G20 | Joint G50 | Interaction-aware G50 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 11.922 | 11.508 | 11.031 | 13.047 | 11.970 | 13.383 | 11.469 | 10.898 |
| 1 | 12.523 | 11.758 | 16.453 | 12.180 | 12.520 | 12.281 | 10.922 | 11.117 |
| 2 | 11.352 | 11.312 | 14.523 | 12.492 | 11.350 | 12.156 | 11.336 | 11.242 |
...
| Depth → Joint, warm-started | Depth + uniform 3-bit q_proj | -0.427 ± 0.365 | 3/3 |
| Depth → Joint, warm-started | Independent depth + searched q_proj quant | -0.421 ± 0.364 | 3/3 |
| Depth → Joint, warm-started | Standard joint G20 | -1.081 ± 0.706 | 3/3 |
| Depth → Joint, warm-started | Standard joint G50 | +0.284 ± 0.479 | 1/3 |
| Depth → Joint, warm-started | Interaction-aware joint G50 | +0.440 ± 0.321 | 0/3 |
...
| Quantization → Joint, warm-started | Depth + uniform 3-bit q_proj | +0.620 ± 0.832 | 1/3 |
| Quantization → Joint, warm-started | Independent depth + searched q_proj quant | +0.626 ± 0.838 | 1/3 |
| Quantization → Joint, warm-started | Standard joint G20 | -0.034 ± 0.341 | 2/3 |
| Quantization → Joint, warm-started | Standard joint G50 | +1.331 ± 0.220 | 0/3 |
| Quantization → Joint, warm-started | Interaction-aware joint G50 | +1.487 ± 0.580 | 0/3 |
...
| Depth → Joint, warm-started | 9.3 | 0 | 7 | 0 |
| Quantization → Depth, frozen | 13.0 | 2 | 7 | 0 |
| Quantization → Joint, warm-started | 16.3 | 1 | 6 | 0 |

## Interpretation

- Depth → Joint warm-starting improves on independent composition by 0.421 PPL on average and wins 3/3 paired seeds. It also has the lowest sample SD among the sequential variants.
...
- Quantization → Joint warm-starting is close to standard joint G20 on mean PPL, but does not show the consistent benefit observed for depth-first warm-starting.
...
- Interaction-aware G50 is reported as a separate method reference. Its 50-generation budget and mutation operator differ from the G20 standard operator used in every sequential stage-two run.
