#!/usr/bin/env python3
"""Parse generation-wise metrics from evo_quant_search.py stdout."""
...
PARENT_BITS_RE = re.compile(r"^Parent bits:\s*(\d+)$")
BIT_AVERAGE_RE = re.compile(rf"^Bit average:\s*({NUMBER})$", re.IGNORECASE)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert evo_quant_search.py stdout into generation-wise CSV metrics."
    )
    parser.add_argument("--log", required=True, help="Quant-search stdout log to parse.")
...
def parse_integer_list(text: str) -> list[int]:
    try:
        ...
        raise ValueError(f"Invalid quantization search-point list: {text}") from exc
...
def encode_search_point(groups: list[list[int]]) -> str:
...
def parse_log(log_file: Path, run_id: str, allow_incomplete: bool = False) -> tuple[list[dict[str, str]], int]:
    ...
    generation_groups: dict[int, list[list[int]]] = {}
    expected_generations: int | None = None
    current_generation: int | None = None
    reading_generation_point = False
    saw_final_configuration = False
    reading_final_point = False
    ...
    for raw_line in log_file.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if line == "Current search point:" and current_generation is not None:
            reading_generation_point = True
            continue

        if line == "Final configuration:":
            saw_final_configuration = True
            reading_final_point = True
            reading_generation_point = False
            current_generation = None
            continue

        if line.startswith("["):
            values = parse_integer_list(line)
            if reading_final_point:
                final_groups.append(values)
                continue
            if reading_generation_point and current_generation is not None:
                generation_groups[current_generation].append(values)
                continue

        if reading_generation_point and current_generation is not None:
            reading_generation_point = False
            groups = generation_groups[current_generation]
            if groups:
                generation_rows[current_generation]["search_point"] = encode_search_point(groups)

        if reading_final_point:
            reading_final_point = False
            if final_groups:
                final_row["search_point"] = encode_search_point(final_groups)

        parent_bits_match = PARENT_BITS_RE.match(line)
        if parent_bits_match and current_generation is not None:
            generation_rows[current_generation]["parent_bits"] = parent_bits_match.group(1)
            continue

        bit_average_match = BIT_AVERAGE_RE.match(line)
        if bit_average_match and current_generation is not None:
            generation_rows[current_generation]["bit_average"] = bit_average_match.group(1)
            continue
        ...
        selection_match = SELECTION_TRAIN_FITNESS_RE.match(line)
        if selection_match and current_generation is not None:
            generation_rows[current_generation]["selection_train_fitness"] = selection_match.group(1)
            continue
    ...
    if current_generation is not None and generation_groups[current_generation]:
        generation_rows[current_generation]["search_point"] = encode_search_point(
            generation_groups[current_generation]
        )
    if final_groups:
        final_row["search_point"] = encode_search_point(final_groups)
    ...
    for generation in observed_generations:
        ...
        missing = [
            field
            for field in ("train_fitness", "bit_average", "parent_bits", "search_point")
            if not row[field]
        ]
        if missing:
