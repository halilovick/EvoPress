# Evolutionary search over non-uniform unstructured sparsity profiles.
#
# This script assumes that prune.py has already generated a sparse weight
# database. For each prunable linear layer, that database contains multiple
# sparse versions of the layer weight, saved as:
#
#   sparse_weights_path/<layer_name>/<level>.pth
#
# Level 0 corresponds to the target average sparsity generated by prune.py.
# Negative levels are denser / less sparse, and positive levels are more sparse.
#
# A candidate solution is represented as an integer vector with one entry per
# prunable layer. Each entry selects which saved sparsity level should be loaded
# for that layer.
#
# The search starts from the uniform profile [0, 0, ..., 0]. Mutations preserve
# the global sparsity budget by decreasing one layer's level and increasing
# another layer's level. This transfers sparsity from sensitive layers to less
# sensitive layers while keeping the average compression approximately fixed.
#
# Candidate profiles are evaluated using perplexity or KL divergence against
# dense-model teacher logits. Multi-stage selection first evaluates many
# offspring cheaply on few tokens, then reevaluates fewer candidates on more
# tokens, adding the parent in the final stage for elitism.
...
def load_layers(model: AutoModelForCausalLM, layer_names: List[str], new_state: List[int], sparse_weights_path: str):
    assert hasattr(model, "state") # model.state tracks which sparse level is currently loaded for that layer
    for layer_name, new_level, old_level in zip(layer_names, new_state, model.state):
        if new_level != old_level:
            ...
            ).to(layer.weight.dtype) # load up new layer weights from file  
    # Update model state
...
def main():
    ...
    # stores dense model outputs before any sparse weights
    # then sparse models are compared to this
    target_logits = []
    ...
    parent = [0 for _ in layer_names] # initial sparsity state (all 0)
    ...
    for generation in range(args.generations):
        ...
        for num_survive, num_tokens in zip(args.survivors_per_selection, args.tokens_per_selection):
            ...
            # example:
            # 64 offspring -> evaluate on 2048 tokens, keep 8
            # 8 candidates -> evaluate on 16384 tokens, keep 2
            # 2 candidates + parent -> evaluate on 65536 tokens, keep 1
